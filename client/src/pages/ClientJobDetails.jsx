...
const ClientJobDetails = () => {
  ...
  const fetchFreelancerProfile = async (freelancerId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/user-profile/${freelancerId}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p className="text-gray-400 text-sm mb-6">Add a new milestone for this job</p>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] pb-[39px] text-white">
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between w-full">
            <div>
              ...
              <p className="text-gray-400 mt-1">Job Details & Milestone Management</p>
            </div>
            {job?.hiredFreelancer && (
              <button
        ...
        {job && (
          <>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-[#2d2d3a] p-4 rounded-lg">
                  ...
                  <p className="text-lg font-bold">PKR {job.budget?.toLocaleString()}</p>
                </div>
                ...
                <div className="bg-[#2d2d3a] p-4 rounded-lg">
                  ...
                  <p className="text-lg font-bold">{formatDate(job.deadline)}</p>
                </div>
                ...
                <div className="bg-[#2d2d3a] p-4 rounded-lg">
                  ...
                  <p className="text-lg font-bold">{job.milestones?.length || 0}</p>
                </div>
              </div>
              ...
              >
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-bold">{freelancerProfile?.name}</h3>
                    ...
                    {freelancerProfile && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        ...
                        {freelancerProfile.skills && freelancerProfile.skills.length > 0 && (
                          <div>
                            <h4 className="text-sm font-medium text-gray-400 mb-1">Skills</h4>
                            <div className="flex flex-wrap gap-2">
                              {freelancerProfile.skills.slice(0, 5).map((skill, index) => (
                                <span key={index} className="bg-[#9333EA]/20 text-[#9333EA] px-2 py-1 rounded text-xs">
                                  {skill}
                                </span>
                              ))}
                            </div>
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                  <span className="text-sm text-gray-400">
                    {job.milestones?.filter((m) => m.status === "approved").length || 0} completed
                  </span>
                  <span className="text-gray-600">•</span>
                  <span className="text-sm text-gray-400">
                    {job.milestones?.filter((m) => m.status === "submitted").length || 0} pending review
                  </span>
                </div>
              </div>
              ...
              {/* Active Milestones */}
              <div className="mb-8">
                <h3 className="text-lg font-medium text-gray-300 mb-4">Active Milestones</h3>
                {job.milestones?.filter((m) => m.status === "in-progress").length === 0 ? (
                  <p className="text-gray-400 text-center py-8">No active milestones</p>
                ) : (
                  <div className="space-y-4">
                    {job.milestones
                      ?.filter((m) => m.status === "in-progress")
                      .map((milestone) => (
                        <div key={milestone._id} className="bg-[#2d2d3a] rounded-lg border-l-4 border-blue-500">
                            ...
                            <div className="flex justify-between items-center">
                              ...
                              <div className="flex items-center gap-4">
                                <div className="text-right">
                                  <p className="text-sm font-medium">PKR {milestone.amount.toLocaleString()}</p>
                                  <p className="text-xs text-gray-400">{formatDate(milestone.deadline)}</p>
                                </div>
                                <ChevronDown
                                  size={18}
              ...
              <div className="mb-8">
                <h3 className="text-lg font-medium text-gray-300 mb-4">Submitted Milestones</h3>
                {job.milestones?.filter((m) => m.status === "submitted").length === 0 ? (
                  <p className="text-gray-400 text-center py-8">No submitted milestones</p>
                ) : (
                  <div className="space-y-4">
                    {job.milestones
                      ?.filter((m) => m.status === "submitted")
                      .map((milestone) => (
                        <div key={milestone._id} className="bg-[#2d2d3a] rounded-lg border-l-4 border-yellow-500">
                          <div className="p-4">
                            <div className="flex justify-between items-start mb-4">
                              <div>
                                ...
                                <div className="flex items-center gap-4 mt-2">
                                  <span className="text-sm">PKR {milestone.amount.toLocaleString()}</span>
                                  <span className="text-sm text-gray-400">{formatDate(milestone.deadline)}</span>
                            ...
                            {milestone.submission && (
                              <div className="bg-[#1c1c24] rounded-lg p-4 mb-4">
                                ...
                                {milestone.submission.message && (
                                  <p className="text-sm text-gray-300 mb-3">{milestone.submission.message}</p>
                                )}
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-300 mb-4">Completed Milestones</h3>
                {job.milestones?.filter((m) => m.status === "approved").length === 0 ? (
                  <p className="text-gray-400 text-center py-8">No completed milestones</p>
                ) : (
                  <div className="space-y-4">
                    {job.milestones
                      ?.filter((m) => m.status === "approved")
                      .map((milestone) => (
                        <div key={milestone._id} className="bg-[#2d2d3a] rounded-lg border-l-4 border-green-500">
                          <div className="p-4">
      ...
      <AnimatePresence>
        {newMilestoneOpen && <NewMilestoneForm onClose={() => setNewMilestoneOpen(false)} />}
      </AnimatePresence>
