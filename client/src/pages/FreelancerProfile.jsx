...
const FreelancerProfile = () => {
    ...
    const fetchAccountStats = async () => {
      try {
        ...
        const [jobsResponse, paymentsResponse] = await Promise.all([
          axios.get(`http://localhost:5000/api/jobs/user/${userId}`, {
        ...
        setAccountStats({ completedJobs, totalEarnings });
      } catch (error) {
  ...
  const handleImageChange = async (file) => {
    ...
    try {
      ...
      if (
        error.message ===
        "File size too large. Got 10822574. Maximum is 10485760."
      ) {
        const maxSizeMB = (10485760 / (1024 * 1024)).toFixed(2);
        setError(
          `File size too large. Maximum allowed size is ${maxSizeMB} MB. Please choose a smaller file.`
        );
      } else {
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {formData.skills.map((skill, index) => (
                        <div
                          key={index}
                          className="bg-[#1e1e2d] text-white px-3 py-1 rounded-full flex items-center"
                        >
                          {skill}
                          <button
                            type="button"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {profileData?.skills && profileData.skills.length > 0 ? (
                      profileData.skills.map((skill, index) => (
                        <span
                          key={index}
                          className="bg-[#1e1e2d] text-white px-3 py-1 rounded-full"
                        >
                          {skill}
                        </span>
                      ))
                  ...
                  <div className="space-y-3">
                    ...
                    profileData.languages.length > 0 ? (
                      profileData.languages.map((language, index) => (
                        <div
                          key={index}
                          className="flex items-center justify-between bg-[#1e1e2d] p-3 rounded-lg"
                        >
                          <div className="flex items-center">
                            <Languages
                              size={18}
                              className="text-[#9333EA] mr-3"
                            />
                  ...
                  <div className="space-y-6">
                    ...
                    profileData.experience.length > 0 ? (
                      profileData.experience.map((exp, index) => (
                        <div
                          key={index}
                          className="border-b border-[#2d2d3a] last:border-b-0 pb-6 last:pb-0"
                        >
                ...
                <div className="bg-[#121218] rounded-lg max-w-2xl w-full mx-4 overflow-hidden">
                  ...
                  <div className="p-6 max-h-[80vh] overflow-y-auto">
                    ...
                    <p className="text-gray-400 mb-4">
                      {formatDate(selectedPortfolio.startDate)} -{" "}
                      {selectedPortfolio.endDate
                        ? formatDate(selectedPortfolio.endDate)
                        : "Present"}
                    </p>
                            ...
                            {selectedPortfolio.images
                              ...
                              .map((image, index) => (
                                <img
                                  key={index}
                                  src={image}
                                  alt={`${selectedPortfolio.title} image ${
                                    index + 2
                                  }`}
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-[#1e1e2d] p-4 rounded-lg">
                  ...
                  <p className="text-xl font-bold mt-1">
                    PKR{" "}
                      {profileData?.payments?.available?.toLocaleString() ||
                        "0"}
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold">
                      PKR{" "}
                      {profileData?.payments?.inProgress?.toLocaleString() ||
                        "0"}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold">
                      PKR{" "}
                      {profileData?.payments?.pending?.toLocaleString() || "0"}
                    </span>
                  </div>
